resource "aws_lb" "frontend_alb" {
  ...
  enable_deletion_protection = false # if you give true it will not be deleted permanently
...
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name    = "roboshop-${var.environment}.${var.domain_name}" # roboshop-dev.awsdevops2025.fun
  type    = "A"
